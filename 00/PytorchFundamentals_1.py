...
"""
torch.cuda --> Used for GPU mostly
torch.float32 or torch.float or float64, etc.. is used for precision in computing
"""
float_32_tensor = torch.tensor([3.0, 6.0, 9.0], dtype=None, device=None, requires_grad=False)
# (dtype) defaults to None, which is torch.float32 or whatever datatype is passed
# (device) defaults to None, which uses the default tensor type (cpu)
# (requires_grad) if True, operations performed on the tensor are recorded 

print("float32 tensor: ", float_32_tensor)

"""
Main attributes that can be attained from tensors are the following:
